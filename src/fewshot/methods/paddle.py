...
class KM(object):
    ...
    def __init__(self, model, device, log_file, args):
        ...
        self.covariance_used = args.covariance_used
        self.ckpt_path = args.ckpt_path
        self.s_use_all_train_set = args.s_use_all_train_set
        self.select_support_elements = type(args.select_support_nb_elements) == int
        self.select_support_nb_elements = args.select_support_nb_elements
        self.predire_une_seule_classe = args.predire_une_seule_classe
        self.args = args
    ...
    def get_logits(self, samples, use_s=True):
        ...
        if use_s:
            ...
            logits = (
                1 / 2 * ((self.w.unsqueeze(1) - samples.unsqueeze(2)) ** 2).sum(dim=-1)
            )  # N x n x K (ie n_task x n_query x num_class)
        return logits

    def get_logits_avec_S(self, samples):
        logits = (self.w.unsqueeze(1) - samples.unsqueeze(2)) ** 2 * self.s.unsqueeze(1)
        logits = 1 / 2 * logits.sum(3) - 1 / 2 * torch.log(
            self.s.unsqueeze(1) + 1e-6
        ).sum(3)
        return logits
    ...
    def get_logits_avec_S_full(self, samples):
        ...
        logits = torch.zeros(n_task, n_query, num_class).to(
            self.device
        )  ## size: [n_query x num_class] on row probs for each class
        for a in range(n_task):
            for n in range(n_query):
                for k in range(num_class):
                    logits[a, n, k] = torch.matmul(
                        (samples[a, n] - self.w[a, k]).unsqueeze(0),
                        torch.matmul(self.s[a, k], samples[a, n] - self.w[a, k]),
                    )

        # diff = self.w.unsqueeze(1) - samples.unsqueeze(2)  # N x n x K x C
        # logits = ((diff.square_()).mul_(self.s.unsqueeze(1))).sum(dim=-1)

        return 0.5 * logits - 0.5 * torch.logdet(self.s)
    ...
    def record_info(self, y_q):
        ...
        for i in range(len(preds_q)):
            cf_matrix[0, :, :] += confusion_matrix(
                y_q[i].cpu().numpy().flatten(),
                preds_q[i].cpu().numpy().flatten(),
                labels=list(range(a)),
            )
    ...
    def run_task(self, task_dic, all_features_support, all_labels_support, shot, gamma=1.0, ):
        ...
        #if same_query_size and (n_query != y_q.shape[1]):
            #return "Pas assez de query examples disponibles", [], [], []
        ...
        # query, support = scaler(query, support,both=True)
        # query, support = scaler(query, support)
        # support, query = scaler(support, query)
        # to remove

        # Run adaptation
        if self.predire_une_seule_classe:
            truth, prediction, confidence = self.run_method_1_classe(
                support,
                query,
                y_s,
                y_q,
            ...
            truth, prediction, confidence = self.run_method(
                support,
                query,
                y_s,
                y_q,
...
class PADDLE(KM):
    ...
    def u_update(self, query):
        ...
        self.u = (-logits + self.alpha * self.A_adj(self.v, n_query)).softmax(
            2
        )  ## the A_adj creates the matrix of pi by repeating over rows since the same for each sample
        ## u is size [n_query x num_class] on row probs for each class
        ## v corresponds to the [log(pi)]k in the paper
    ...
    def v_update(self):
        ...
        """
        self.v = (
            torch.log(self.A(self.u) + 1e-6) + 1
        )  ## A transforms U to probability distribution over classes (sum over all samples)
    ...
    def w_update(self, support, query, y_s_one_hot, gamma):
        ...
        if self.select_support_elements:
            num += gamma * (
                support.unsqueeze(2) * (y_s_one_hot * self.xi).unsqueeze(3)
            ).sum(1)
    ...
    def s_update(self, support, query, y_s_one_hot):
        den = (
            (query.unsqueeze(2) - self.w.unsqueeze(1)) ** 2 * self.u.unsqueeze(3)
        ).sum(1) + 1e-6
        if self.select_support_elements:
            den += (
                (support.unsqueeze(2) - self.w.unsqueeze(1)) ** 2
                * (y_s_one_hot * self.xi).unsqueeze(3)
            ).sum(1)
        else:
            den += (
                (support.unsqueeze(2) - self.w.unsqueeze(1)) ** 2
                * y_s_one_hot.unsqueeze(3)
            ).sum(1)
    ...
    def s_init(
        ...
        if self.covariance_used == "sans_S":
            ...
            if self.s_use_all_train_set:
                self.s_init_full_use_all_train_set(all_features_trainset, all_labels_trainset, self.args.cov_matrix_path + self.args.support_split_file + '/')
            else:
    ...
    def s_init_diag(self, support, y_s_one_hot):
        den = (
            (support.unsqueeze(2) - self.w.unsqueeze(1)) ** 2 * y_s_one_hot.unsqueeze(3)
        ).sum(
            1
        )  # + 1e-6
    ...
    def s_init_full_use_all_train_set(self, all_features_trainset, all_labels_trainset, save_dir):
        ...
        if os.path.isfile(filepath):
            ...
            self.s = torch.zeros(n_task, n_ways, feature_dim, feature_dim).to(
                self.device
            )
            ...
            for a in range(n_task):
                for k in range(n_ways):
                    ...
                    covariance = torch.cov(
                        train_set[a].T, aweights=y_all_one_hot[a, :, k]
                    )
                    self.s[a, k] = torch.pinverse(covariance)
    ...
    def s_init_GLASSO_use_all_train_set(self, all_features_trainset, all_labels_trainset, save_dir):
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        ...
        filepath = (
            save_dir
            + "s_covariance_matrix_all_train_set_GLASSO_lambda_"
            + str(lambd_GLASSO)
            + ".plk"
        )
        print(f"loading {filepath}")
        if os.path.isfile(filepath):
            ...
            for a in range(n_task):
                for k in range(n_ways):
                    ...
                    S = utils.GLASSO(C, S_0, lambd_GLASSO, max_iter=20000, eps=1e-6)
                    self.s[a, k] = S
                    plt.figure(figsize=(10, 10))
                    ...
                    plt.colorbar()
                    plt.savefig(
                        save_dir + f"S_{k}_lambd_{lambd_GLASSO}_all_train_set.png"
                    )
    ...
    def s_init_GLASSO(self, support, y_s_one_hot, y_s, save_dir):
        ...
        if os.path.isfile(filepath):
            ...
            for a in range(n_task):
                for k in range(n_ways):
                    features = support[a][y_s[a] == k, ...]
                    C = torch.cov(features.T)
                    if True:  # k!= 1:
                        #print("features", features.shape)
                        # C = torch.cov(features.T, aweights=y_s_one_hot[a,:,k])
                        S_0 = torch.eye(feature_dim)
                        S = utils.GLASSO(
                            C.to("cpu"), S_0, lambd_GLASSO, max_iter=20000, eps=5e-7
                        )
    ...
    def projection_capped_simplex(self, vec_to_project, y_s_one_hot, p):
        ...
        for i in range(n_task):
            for k in range(num_class):
                ...
                proj = self.projection_fast_capped_simplex_1d(
                    vec_to_project[i, index, k].float(), p
                )
    ...
    def projection_capped_simplex_1d(self, y0, p):
        ...
        for a in range(n):
            for b in range(a + 1, n + 1):
                ...
                if (
                    ((a == 0) or (y[a - 1] + gamma <= 0))
                    and (y[a] + gamma > 0)
                    and (y[b - 1] + gamma < 1)
                    and ((b == n) or (y[b] + gamma >= 1))
                ):
                    x[idx[b:]] = 1
    ...
    def projection_fast_capped_simplex_1d(self, y, p):
        ...
        step = 1
        gamma = max(y)
        zero, one = torch.tensor([0.0]).to("cuda"), torch.tensor([1.0]).to("cuda")
        compteur = 0
        gammas_vus = []
        while True:
            ...
            if (
                omega_prime_at_previous_gamma == 0
            ):  # when we are in a flat region, we use our knowledge that the function is increasing
                if omega_at_previous_gamma < 0:
                    gamma = min(y[y > gamma])
                else:
                    gamma = max(y[y < gamma])
            elif (
                abs(step * omega_at_previous_gamma / omega_prime_at_previous_gamma)
                < eps
            ):
                return v_dans_intervalle
            else:
                gamma = gamma - (
                    step * omega_at_previous_gamma / omega_prime_at_previous_gamma
                )
    ...
    def run_method(
        ...
        #print("y_s_one_hot", y_s_one_hot.sum(1))
        n_task, n_examples, n_ways = y_s_one_hot.size()
        feature_dim = support.size(2)

        ## Initialize the centroids with the support samples and support labels
        self.init_w(support=support, y_s=y_s)  ## 5 (classes) x 512 (features)
        ...
        if self.select_support_elements:
            self.xi = torch.full(
                y_s_one_hot.size(),
                self.select_support_nb_elements / (n_task * n_examples),
            ).to(self.device)

        ## initialize the S^ matrix for each k (each one is dxd)
        self.s_init(
            support,
            y_s_one_hot,
            y_s,
        ...
        for i in range(self.iter):
            ...
            ## Update the soft labels based on the current centroids and query samples
            self.u_update(query)

            ## update the log of pi's (class distributions using updated U)
            self.v_update()

            if self.select_support_elements:
                self.xi_update(support, y_s_one_hot, self.select_support_nb_elements)

            # update the centroids using the updated soft labels
        ...
        ## calculate the predictions, confidence, the confusion matrix and the accuracy
        truth, prediction, confidence = self.record_info(y_q=y_q)
        print("pred", prediction)
        print("truth", truth)
        return truth, prediction, confidence

    def run_method_1_classe(
        self,
        support,
        query,
        y_s,
...
class MinMaxScaler(object):
    ...
    def __call__(self, query, support, both=False):
        if both:
            ...
            dist = (
                features.max(dim=1, keepdim=True)[0]
                - features.min(dim=1, keepdim=True)[0]
            )
            dist[dist == 0.0] = 1.0
            scale = 1.0 / dist
