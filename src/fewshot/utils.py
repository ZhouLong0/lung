...
def load_checkpoint(model, model_path, type='best'):
    #model = nn.DataParallel(IndepResNetModel())
    if type == 'best':
        # checkpoint = torch.load('{}/model_best.pth'.format(model_path), map_location=torch.device('cpu'))
...
def extract_features(model, dataset, save_directory, save_filename):
    ...
    with torch.no_grad():
        ...
        all_slices = []
        all_patchs = []
        tqdm_loop = tqdm(dataset)

        for i, (input, label, file_name) in enumerate(tqdm_loop):
            label = torch.Tensor([label])
            outputs = model(input.unsqueeze(0))
            all_features.append(outputs.cpu())
            all_labels.append(label)
            name_slice = file_name.split('_')[0] + '_' + file_name.split('_')[1]
            all_slices.append(name_slice)
            all_patchs.append(file_name)
...
def extract_mean_features(model, train_loader, args, logger, device):
    ...
    # Load features from memory if previously saved ...
    save_dir = os.path.join(args.ckpt_path, 'support')
